...
And also this repo will be converted into private after some time!
...
Также данный репозиторий станет приватным через некоторое время!
...
Репозиторий содержит только базовую версию клиента, а значит здесь находятся только самые необходимые файлы (рекомендую не слишком полагаться на имена файлов и переменных в этом репозитории), программа не была заточена под модификацию - важная информация которую надо упомянуть, хотя некоторые аспекты поддаются лёгкому изменению.
